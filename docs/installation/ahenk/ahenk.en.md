...
The Ahenk package is available at "repo.liderahenk.org.tr". On Pardus computers, this address is defined and the Ahenk package
can be downloaded from the repository. In the terminal (console) to define this repository to your system;

    sudo curl -o /etc/apt/trusted.gpg.d/liderahenk-archive-keyring.asc 'https://repo.liderahenk.org.tr/liderahenk-archive-keyring.asc'

    
The "liderahenk-archive-keyring.asc" key file should be downloaded and installed on the system with the commands. Next;

    sudo printf  "deb [arch=amd64] https://repo.liderahenk.org.tr/liderahenk stable main" | sudo tee -a /etc/apt/sources.list
